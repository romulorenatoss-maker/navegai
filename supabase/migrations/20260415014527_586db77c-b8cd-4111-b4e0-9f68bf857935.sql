...
-- 1. Novo campo peso_recorrencia no template
ALTER TABLE public.operational_templates
ADD COLUMN IF NOT EXISTS peso_recorrencia numeric NOT NULL DEFAULT 1.0;

-- 2. Atualizar templates existentes baseado na recorrencia_tipo
UPDATE public.operational_templates SET peso_recorrencia = CASE recorrencia_tipo
  WHEN 'diaria' THEN 1.0
  WHEN 'semanal' THEN 1.5
  WHEN 'quinzenal' THEN 2.0
  WHEN 'mensal' THEN 3.0
  ELSE 2.0
END;

-- 3. Função: média operacional ponderada por multiplicador
CREATE OR REPLACE FUNCTION public.calcular_media_operacional_ponderada(
  _profile_id uuid,
  _tipo text DEFAULT 'executor',
  _data_inicio timestamptz DEFAULT NULL,
...
AS $$
  SELECT ROUND(
    COALESCE(
      SUM(sl.score_final * COALESCE(t.peso_recorrencia, 1.0)) /
      NULLIF(SUM(COALESCE(t.peso_recorrencia, 1.0)), 0),
      0
    ), 2)
  FROM operational_score_logs sl
  JOIN operational_assignments a ON a.id = sl.assignment_id
  JOIN operational_templates t ON t.id = a.template_id
  WHERE sl.profile_id = _profile_id
    AND sl.tipo_score = _tipo
    AND (_data_inicio IS NULL OR sl.created_at >= _data_inicio)
    AND (_data_fim IS NULL OR sl.created_at <= _data_fim);
$$;

-- 4. Função: nota global consolidada (OS 60% + Oper 40%)
CREATE OR REPLACE FUNCTION public.calcular_nota_global(
  _profile_id uuid,
  _data_inicio timestamptz DEFAULT NULL,
  _data_fim timestamptz DEFAULT NULL
)
RETURNS TABLE(nota_os numeric, nota_operacional numeric, nota_global numeric)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = public
...
BEGIN
  -- Média OS
  SELECT COALESCE(ROUND(AVG(cn.nota), 2), 0) INTO v_nota_os
  FROM calcular_notas_por_setor(_data_inicio, _data_fim) cn
  WHERE cn.profile_id = _profile_id;

  -- Média Operacional (executor como base)
  v_nota_oper := calcular_media_operacional_ponderada(_profile_id, 'executor', _data_inicio, _data_fim);

  RETURN QUERY SELECT
    v_nota_os,
    v_nota_oper,
    ROUND(v_nota_os * 0.6 + v_nota_oper * 0.4, 2);
END;
$$;

-- 5. Atualizar trigger para gravar peso_recorrencia no detalhe_calculo
...
BEGIN
  ...
  v_detalhe_executor := jsonb_build_object(
    ...
    'recorrencia_tipo', v_template.recorrencia_tipo
  );
