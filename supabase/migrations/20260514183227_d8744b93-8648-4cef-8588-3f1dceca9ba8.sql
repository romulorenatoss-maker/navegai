-- ============================================================
-- FASE 8 — Score trigger no fluxo Executor → Avaliado → Aprovador → Auditor
-- Backup físico antes de qualquer alteração.
-- ============================================================

-- 1) Backup completo de operational_score_logs (idempotente)
CREATE TABLE IF NOT EXISTS public.operational_score_logs_backup_fase8
  AS TABLE public.operational_score_logs WITH NO DATA;

INSERT INTO public.operational_score_logs_backup_fase8
SELECT * FROM public.operational_score_logs
WHERE NOT EXISTS (
  SELECT 1 FROM public.operational_score_logs_backup_fase8 b
  WHERE b.id = public.operational_score_logs.id
);

ALTER TABLE public.operational_score_logs_backup_fase8 ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='operational_score_logs_backup_fase8'
      AND policyname='backup_fase8_admin_only'
  ) THEN
    CREATE POLICY backup_fase8_admin_only
      ON public.operational_score_logs_backup_fase8
      FOR ALL
...
-- 2) Trigger atualizada: dispara também em 'aprovada' (final sem auditor)
--    Mantém a mesma fórmula e fanout — apenas amplia a janela de cálculo.
...
BEGIN
  -- Distribuição acontece em CONCLUSÃO real do fluxo:
  --   * 'concluida'                          (com ou sem auditor)
  --   * 'aprovada'  sem auditor designado    (fluxo encerra na aprovação)
  IF NEW.status NOT IN ('concluida','aprovada') THEN RETURN NEW; END IF;
  IF OLD.status = NEW.status THEN RETURN NEW; END IF;
  IF NEW.excluir_da_media = true THEN RETURN NEW; END IF;
  -- Em 'aprovada', só calcula se NÃO houver auditor pendente
  IF NEW.status = 'aprovada'
     AND (NEW.auditor_id IS NOT NULL OR NEW.setor_auditor_id IS NOT NULL) THEN
    RETURN NEW;
  END IF;
  ...
  v_score_aprovador := NEW.score_aprovador;
