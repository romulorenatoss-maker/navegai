...
-- Data: 2026-05-21
-- Escopo: executor -> aprovador -> auditor
-- ============================================================================
-- Corrige referencias ambiguas em funcoes PL/pgSQL quando parametros,
-- variaveis, colunas e RETURNS TABLE usam nomes como assignment_id, field_id,
-- status, id e observacao.
--
-- Nao altera frontend, rotas, menu, layout ou regra de negocio.
-- ============================================================================
...
BEGIN
  ...
  RETURN QUERY
    SELECT p_assignment_id, 'aguardando_aprovacao'::text, v_count;
END;
$$;
...
BEGIN
  ...
  SELECT tpa.*
    ...
    WHERE tpa.id = p_plano_id
      AND tpa.deleted_at IS NULL
    FOR UPDATE;

  IF v_row.id IS NULL THEN
    RAISE EXCEPTION 'Plano nao encontrado ou excluido: %', p_plano_id;
  END IF;
  ...
  UPDATE public.operational_contingencies AS oc
    SET status = 'resolvida',
        ...
        dentro_prazo = CASE
          ...
          WHEN now() <= oc.prazo_resolucao THEN true
          ELSE false
        END
  ...
  SELECT COUNT(*)
    INTO v_pendentes
    FROM public.tarefas_planos_acao_aprovador AS tpa
    WHERE tpa.assignment_id = v_assignment_id
      AND tpa.deleted_at IS NULL
      AND tpa.respondido = false;

  IF v_pendentes = 0 THEN
    UPDATE public.operational_assignments AS oa
      SET status = 'aguardando_aprovacao',
          updated_at = now()
      WHERE oa.id = v_assignment_id
        AND oa.status IN ('devolvida', 'em_andamento');
  END IF;
  ...
  INSERT INTO public.operational_execution_logs (
    ...
    v_assignment_id,
    'executor_respondeu_plano_aprovador',
    v_profile_id,
    jsonb_build_object(
      'plano_id', p_plano_id,
      'rodada', v_row.rodada,
      'planos_restantes', v_pendentes
    )
  );
...
COMMENT ON FUNCTION public.tarefas_rpc_executor_responder_plano_aprovador IS
  'Executor responde plano pendente do aprovador. Resolve contingencias legacy. Status muda quando todos respondidos.';

-- 3. RPC legacy/deprecated: aprovador cria plano de acao
...
BEGIN
  ...
  SELECT COALESCE(MAX(tpa.rodada), 0) + 1
    INTO v_rodada
    FROM public.tarefas_planos_acao_aprovador AS tpa
    WHERE tpa.assignment_id = p_assignment_id
      AND tpa.field_id = p_field_id
      AND tpa.deleted_at IS NULL;
...
COMMENT ON FUNCTION public.tarefas_rpc_aprovador_criar_plano_acao IS
  'DEPRECATED 20260521: usar tarefas_rpc_aprovador_criar_plano_executor. Mantida com aliases qualificados por compatibilidade.';
...
BEGIN
  ...
  INSERT INTO public.operational_execution_logs (
    ...
    p_assignment_id,
    'aprovador_criou_plano_executor',
    v_profile_id,
    jsonb_build_object(
      'field_id', p_field_id,
      'rodada', v_rodada,
...
COMMENT ON FUNCTION public.tarefas_rpc_aprovador_criar_plano_executor IS
  'Aprovador cria plano para executor. Bloqueia se ha plano auditor pendente em outra pergunta.';
...
BEGIN
  ...
  INSERT INTO public.operational_audit_trail (
    ...
    p_assignment_id,
    'aprovador_aprovou_para_auditoria',
    v_profile_id,
  ...
  RETURN QUERY
    SELECT p_assignment_id, 'aguardando_auditoria'::text;
END;
$$;

COMMENT ON FUNCTION public.tarefas_rpc_aprovador_aprovar_para_auditoria IS
  'Aprovador conclui revisao e envia para auditoria. Bloqueia se ha plano pendente.';
...
BEGIN
  ...
  SELECT COUNT(*)
    INTO v_pendentes
    FROM public.tarefas_planos_acao_auditor AS taa
    WHERE taa.assignment_id = v_assignment_id
      AND taa.deleted_at IS NULL
      AND taa.respondido = false;
  ...
  IF v_pendentes = 0 THEN
    UPDATE public.operational_assignments AS oa
      SET status = 'aguardando_auditoria',
          updated_at = now()
      WHERE oa.id = v_assignment_id
        AND oa.status = 'aguardando_aprovacao';
  END IF;
  ...
  INSERT INTO public.operational_execution_logs (
    ...
    v_assignment_id,
    'aprovador_respondeu_plano_auditor',
    v_profile_id,
    jsonb_build_object(
      'plano_id', p_plano_id,
      'rodada', v_row.rodada,
...
-- 7. RPC legacy/deprecated: auditor cria plano de acao
...
BEGIN
  ...
  SELECT COALESCE(MAX(taa.rodada), 0) + 1
    INTO v_rodada
    FROM public.tarefas_planos_acao_auditor AS taa
    WHERE taa.assignment_id = p_assignment_id
      AND taa.field_id = p_field_id
...
COMMENT ON FUNCTION public.tarefas_rpc_auditor_criar_plano_acao IS
  'DEPRECATED 20260521: usar tarefas_rpc_auditor_criar_plano_aprovador. Mantida com aliases qualificados por compatibilidade.';
...
BEGIN
  ...
  UPDATE public.operational_assignments AS oa
    SET status = 'aguardando_aprovacao',
        updated_at = now()
    WHERE oa.id = p_assignment_id
      AND oa.status = 'aguardando_auditoria';
