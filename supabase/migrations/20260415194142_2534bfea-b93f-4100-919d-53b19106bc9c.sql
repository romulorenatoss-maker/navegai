...
-- 1. Add penalty config fields to operational_templates
ALTER TABLE public.operational_templates 
  ADD COLUMN IF NOT EXISTS penalidade_contingencia numeric NOT NULL DEFAULT 10,
  ADD COLUMN IF NOT EXISTS penalidade_sla_contingencia numeric NOT NULL DEFAULT 15,
  ADD COLUMN IF NOT EXISTS habilitar_perguntas_automaticas boolean NOT NULL DEFAULT true;

-- 2. Add fields to operational_contingencies
ALTER TABLE public.operational_contingencies
  ADD COLUMN IF NOT EXISTS prazo_resolucao timestamptz,
  ADD COLUMN IF NOT EXISTS motivo_instrucao text,
  ADD COLUMN IF NOT EXISTS dentro_prazo boolean,
  ADD COLUMN IF NOT EXISTS numero_contingencia serial,
  ADD COLUMN IF NOT EXISTS impacto_score numeric DEFAULT 0;

-- 3. Create full assignment history table
CREATE TABLE IF NOT EXISTS public.operational_assignment_history (
...
ALTER TABLE public.operational_assignment_history ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can view assignment history"
  ON public.operational_assignment_history FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "Authenticated users can insert assignment history"
  ON public.operational_assignment_history FOR INSERT
  TO authenticated WITH CHECK (true);

CREATE INDEX IF NOT EXISTS idx_op_history_assignment ON public.operational_assignment_history(assignment_id);
CREATE INDEX IF NOT EXISTS idx_op_history_tipo ON public.operational_assignment_history(tipo_evento);

-- 4. Update score trigger to apply contingency penalties
...
BEGIN
  ...
  -- Count contingencies resolved outside SLA
  ...
  -- === SCORE EXECUTOR with contingency penalties ===
  ...
  -- Apply contingency penalties to avaliado too
  IF v_total_contingencias > 0 THEN
  ...
  -- Update dentro_prazo on contingencies
