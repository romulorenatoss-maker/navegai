-- Persistencia das etapas do executor atual.
-- Nao usa operational_execution_step_logs porque ela aponta para o modelo legado
-- operational_template_steps; o executor atual trabalha por template_snapshot.sections.
...
BEGIN
  ...
  IF v_assignment.setor_executor_id IS NOT NULL AND v_profile_id IS NOT NULL THEN
    SELECT EXISTS (
      ...
      WHERE cs.profile_id = v_profile_id
        AND cs.setor_id = v_assignment.setor_executor_id
    ) INTO v_has_setor;
  END IF;
  ...
  SELECT *
  INTO v_run
  FROM public.operational_assignment_stage_runs
  ...
  INSERT INTO public.operational_assignment_stage_runs (
    ...
    started_at,
    inicio_atrasado,
    inicio_atraso_minutos,
    started_by
  )
  VALUES (
    p_assignment_id,
    p_stage_id,
    COALESCE(NULLIF(p_stage_label, ''), 'Etapa'),
    COALESCE(p_stage_order, 0),
    p_horario_inicio_previsto,
    p_horario_fim_previsto,
    'em_andamento',
    now(),
    v_delay_minutes > 0,
    GREATEST(v_delay_minutes, 0),
    v_profile_id
  )
  ...
  SET
    status = CASE
      WHEN status IN ('aberta', 'pendente', 'reaberta', 'devolvida') THEN 'em_andamento'
      ELSE status
    END,
    ...
    justificativa_sla_etapa = CASE
      WHEN v_run.inicio_atrasado THEN COALESCE(justificativa_sla_etapa, 'Inicio de etapa fora do horario previsto')
      ELSE justificativa_sla_etapa
    END,
    updated_at = now()
  WHERE id = p_assignment_id;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'inicio_atrasado', v_run.inicio_atrasado,
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Etapa ainda nao iniciada';
  END IF;

  IF v_run.status = 'concluida' THEN
    RETURN v_run;
  END IF;
  ...
  UPDATE public.operational_assignments
  SET
    flag_sla_etapa_estourado = COALESCE(flag_sla_etapa_estourado, false) OR v_run.inicio_atrasado OR v_run.fim_atrasado,
    justificativa_sla_etapa = CASE
      WHEN v_run.fim_atrasado THEN COALESCE(justificativa_sla_etapa, 'Finalizacao de etapa fora do horario previsto')
      ELSE justificativa_sla_etapa
    END,
    updated_at = now()
  WHERE id = p_assignment_id;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'fim_atrasado', v_run.fim_atrasado,
      'fim_atraso_minutos', v_run.fim_atraso_minutos,
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT (assignment_id, field_id, versao)
    DO UPDATE SET
...
BEGIN
  ...
  UPDATE public.operational_assignments oa
