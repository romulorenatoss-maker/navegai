...
export const VALID_TRANSITIONS: Record<string, string[]> = {
  ...
  [TASK_STATUS.EM_ANDAMENTO]: [
    TASK_STATUS.AGUARDANDO_APROVACAO,    // saneamento 4 papéis: executor → aprovador (sem etapa intermediária)
  ...
  // Saída segura para registros legados (deprecated): só conseguem ir para aprovação ou devolvida.
  [TASK_STATUS.AGUARDANDO_AVALIACAO]: [TASK_STATUS.AGUARDANDO_APROVACAO, TASK_STATUS.DEVOLVIDA],
  [TASK_STATUS.EM_AVALIACAO]: [TASK_STATUS.AGUARDANDO_APROVACAO, TASK_STATUS.DEVOLVIDA],
  [TASK_STATUS.CONTINGENCIADO]: [TASK_STATUS.AGUARDANDO_APROVACAO, TASK_STATUS.EM_ANDAMENTO],
  [TASK_STATUS.AGUARDANDO_APROVACAO]: [TASK_STATUS.APROVADA, TASK_STATUS.DEVOLVIDA, TASK_STATUS.CONCLUIDA, TASK_STATUS.AGUARDANDO_AUDITORIA],
  [TASK_STATUS.AGUARDANDO_AUDITORIA]: [TASK_STATUS.APROVADA, TASK_STATUS.CONCLUIDA, TASK_STATUS.DEVOLVIDA],
  [TASK_STATUS.AGUARDANDO_VALIDACAO]: [
    TASK_STATUS.APROVADA,
    TASK_STATUS.CONCLUIDA,
    TASK_STATUS.DEVOLVIDA,
    TASK_STATUS.EM_PLANO_ACAO,
    TASK_STATUS.AGUARDANDO_APROVACAO, // saneamento 4 papéis: solicitante encaminha pro aprovador, sem avaliação
  ],
  [TASK_STATUS.DEVOLVIDA]: [TASK_STATUS.EM_ANDAMENTO, TASK_STATUS.CANCELADA, TASK_STATUS.AGUARDANDO_APROVACAO],
...
const ROLE_FOR_TARGET: Record<string, OperationalRole[]> = {
  ...
  [TASK_STATUS.CONTINGENCIADO]:         ["EXECUTOR", "APROVADOR", "ADMIN"],
  [TASK_STATUS.AGUARDANDO_APROVACAO]:   ["EXECUTOR", "APROVADOR", "ADMIN"], // executor envia para aprovação; aprovador também pode (saneamento)
  [TASK_STATUS.APROVADA]:               ["APROVADOR", "CRIADOR_DESIGNANTE", "ADMIN"],
  [TASK_STATUS.CONCLUIDA]:              ["APROVADOR", "CRIADOR_DESIGNANTE", "EXECUTOR", "ADMIN"],
  [TASK_STATUS.DEVOLVIDA]:              ["APROVADOR", "CRIADOR_DESIGNANTE", "ADMIN"],
  [TASK_STATUS.REPROVADA]:              ["APROVADOR", "ADMIN"],
  [TASK_STATUS.EM_PLANO_ACAO]:          ["CRIADOR_DESIGNANTE", "APROVADOR", "ADMIN"],
