...
export function StepResumo({
  ...
  const pesoValidador = validadorChecks.reduce((s, i) => s + (i.peso || 0), 0);

  // Detecta se Avaliador e Aprovador são o mesmo usuário (consolidação de governança).
  const avaliadorAprovadorMesmo = !!form.avaliador_profile_id
    && form.avaliador_profile_id === form.aprovador_profile_id;
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className={cardCls}>
          ...
          {avaliadorAprovadorMesmo && (
            <p className="mt-2 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1">
              Avaliador e Aprovador são o mesmo usuário — Nota de Governança será <strong>consolidada</strong> (sem duplicar SLA/penalidade).
            </p>
          )}
        </div>
      ...
      <div className="border border-border rounded-lg bg-card p-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
          ...
          <div className={`rounded border ${hasAprovador ? "border-blue-200 bg-blue-50/50" : "border-border bg-muted/30 opacity-60"} p-2`}>
            ...
            <div className="text-foreground">Pertence ao <strong>Avaliador / Aprovador</strong></div>
            <div className="text-muted-foreground mt-1">
              ...
              {avaliadorAprovadorMesmo && " · consolidado"}
            </div>
          </div>
          <div className={`rounded border ${hasValidador ? "border-purple-200 bg-purple-50/50" : "border-border bg-muted/30 opacity-60"} p-2`}>
