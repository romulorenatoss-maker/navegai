...
export default function QuickTaskDialog({ open, onOpenChange, defaultAvaliadoId, defaultResponsavelId, taskType = "inspecao", initialSetorId = "", origemContexto = "rotina" }: Props) {
  ...
  const derivedSetorIdFromAvaliado = useMemo(() => {
    ...
    if (av.mode === "setor_todo") return av.setorId || "";
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const templatePayload: any = {
        ...
        // Saneamento 4 papéis: campos legados validador_* (pré-aprovação) consolidados em aprovador_*.
        aprovador_profile_id: aprovacaoAtiva && aprovadorMode === "individual" ? (aprovadorId || null)
          : (requerValidacao && validadorMode === "individual" ? (validadorId || null) : null),
        aprovador_setor_id: aprovacaoAtiva && aprovadorMode === "setor" ? (aprovadorSetorId || null)
          : (requerValidacao && validadorMode === "setor" ? (validadorSetorId || null) : null),
      ...
      const snapshot = {
        ...
        responsaveis: {
          ...
          // Saneamento 4 papéis: deduplicados validador_*/aprovador_* (mesmo papel APROVADOR).
        ...
        responsaveis_multi: {
          ...
          avaliado: respBlocks.avaliado,
          avaliador: respBlocks.avaliador,
          aprovadorFinal: respBlocks.aprovadorFinal,
          validadorFinal: respBlocks.validadorFinal,
        },
      };
      ...
      const assignPayload: any = {
        ...
        // Saneamento 4 papéis: validador_*/aprovador_* consolidados.
        aprovador_id: aprovacaoAtiva && aprovadorMode === "individual" ? (aprovadorId || null)
          : (requerValidacao && validadorMode === "individual" ? (validadorId || null) : null),
        setor_aprovador_id: aprovacaoAtiva && aprovadorMode === "setor" ? (aprovadorSetorId || null)
          : (requerValidacao && validadorMode === "setor" ? (validadorSetorId || null) : null),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[96vw] h-[92vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="px-5 pt-5 pb-3 border-b border-border shrink-0">
          <DialogTitle className="text-base">
            {taskType === "simples" ? "Tarefa Avulsa Simples" : "Tarefa Avulsa por Etapa"}
          </DialogTitle>
          {/* Stepper */}
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="border border-border rounded-lg p-3 space-y-3 bg-muted/30">
                ...
                {isSelfTask && respBlocks.aprovadorFinal.profileIds.includes(profile?.id || "") && (
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              <TabFormBuilder
                ...
                setFields={setFields}
                setores={setores as any[]}
                tipoExecucao={taskType === "simples" ? "simples" : "etapas"}
