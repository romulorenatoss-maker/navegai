...
// Aba de recorrência, SLA e datas. Código completo e autossuficiente.
...
export function RotinasTabRotina({ form, set, onSave, saving }: Props) {
  const dates = useMemo(() => gerarDatas(form), [
    ...
    form.data_fim, form.repetir_sempre,
  ]);
    ...
    <div className="space-y-5 p-1">
        ...
        <div className="flex items-center gap-3 bg-muted/50 rounded-lg border border-border p-3">
          <Switch
            ...
            onCheckedChange={(v) => {
              ...
              if (v) { const t = getLocalToday(); set("data_inicio", form.data_inicio >= t ? form.data_inicio : t); set("data_fim", ""); }
            }}
          />
          <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {DIAS_SEMANA.map((d, i) => (
              <button key={i} type="button" onClick={() => toggleDia(i)}
                className={`px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${form.dias_da_semana.includes(i)
      ...
      <div className="grid grid-cols-2 gap-4 border-t border-border pt-4">
        <div className="space-y-1.5">
          ...
          <p className="text-[10px] text-muted-foreground">Tempo máximo para o executor concluir a tarefa.</p>
        </div>
        <div className="space-y-1.5">
            ...
            {Object.entries(grouped).map(([month, monthDates]) => (
              <div key={month}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
                  {monthDates.map((d, i) => (
                    <div key={i} className="flex items-center gap-2 bg-card border border-border rounded px-2 py-1.5 text-xs">
