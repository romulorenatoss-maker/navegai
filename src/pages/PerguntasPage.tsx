...
export default function PerguntasPage() {
  ...
  const { data: perguntas = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("perguntas_avaliacao").select("*, tipos_servico(nome)").order("ordem");
  ...
  const openCreate = () => {
    setEditing(null); setPergunta(""); setTipoServicoId(""); setTipoAvaliado("atendente"); setPeso("1"); setOrdem(String(perguntas.length));
    setDialogOpen(true);
  };
  const openEdit = (p: Pergunta) => {
    setEditing(p); setPergunta(p.pergunta); setTipoServicoId(p.tipo_servico_id || ""); setTipoAvaliado(p.tipo_avaliado); setPeso(String(p.peso)); setOrdem(String(p.ordem));
    setDialogOpen(true);
  };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-body text-muted-foreground">Cadastro e ordenação de perguntas por tipo de serviço.</p>
        </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-8 text-center text-body text-muted-foreground">Nenhuma pergunta cadastrada.</td></tr>
            ) : perguntas.map((p, i) => (
