...
interface OSSectionProps {
  ...
  items: OSWithProgress[];
  emptyText: string;
  defaultOpen?: boolean;
  onClickOS: (os: OSWithProgress) => void;
}

function OSSection({ title, icon, items, emptyText, defaultOpen = true, onClickOS }: OSSectionProps) {
  ...
  return (
    <motion.div variants={itemVariants} initial="hidden" animate="show" className="bg-card border border-border rounded-lg shadow-card">
      ...
      >
        {icon}
        <h2 className="text-body font-semibold text-foreground flex-1 text-left">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-body text-muted-foreground">{emptyText}</td>
                </tr>
              )}
            </tbody>
          </table>
